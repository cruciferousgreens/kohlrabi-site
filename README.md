...
- **Links between pages are relative** (`features.html`, not full URLs), so local previews and PR preview links work.
...
- **`/workouts/*` and `/programs/*`** are served by `_redirects` 200-rewrites to the generic `/workout` and `/program` pages; the slug is read from the URL path.
