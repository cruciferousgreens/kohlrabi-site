...
- **`/workouts/*` and `/programs/*`** are generated as static per-slug detail pages at build time (`workouts/<slug>.html`, `programs/<slug>.html`), each with a unique title/description, self-referential canonical, and `ExercisePlan` JSON-LD. There are deliberately NO `_redirects` rewrites for these paths: Pages evaluates `_redirects` before static assets, so a wildcard 200 rule would shadow every detail page with the generic shell. Unknown slugs 404.
